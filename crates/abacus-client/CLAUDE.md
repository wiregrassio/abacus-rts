...
| `interlock.rs` | Interlock (creator r/w, free(), stop_touch_thread()), AttachedInterlock (attacher r/w counters, r/o expiration, free()), AttachedWaitCounter (r/o), ClockHandle (r/o). |
| `wait_counter.rs` | WaitCounter: wait_until(target, timeout_ms), CAS-max on open_count, reaped detection, free(). |
| `wait_timer.rs` | WaitTimer: wait_ms with absolute 2*W deadline, RTSTimeout abort, wait_until sugar, free(). CAS-max on open_count. |
| `wait_cron.rs` | WaitCron: recurring grid-aligned wait, snapshot-based wake detection, free(). |
| `wait_barrier.rs` | WaitBarrier: multi-condition wait, daemon-evaluated, free(). |
| `wait_race.rs` | WaitRace: first-of-N polling over WaitCounters. SDK-only, no daemon changes. |
| `process_clock.rs` | ProcessClock: liveness + uptime beacon, free(). Custom touch thread updates open_count to current time. |
...
- All SDK types expose free() for voluntary termination via SENTINEL.
- All wait paths check for reaped state and return Err(InterlockReaped). All wait paths check for SENTINEL.
...
- Counter increments use fetch_add (not saturating).
...
- interlock_free lives in `abacus-core`, not `abacus-daemon`. free() does not require daemon communication and is unaffected by the wire codec coupling above.
</known-hazards>
